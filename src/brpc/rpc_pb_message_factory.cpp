...
DEFINE_bool(enable_protobuf_arena_for_rpc, false,
            "Enable protobuf arena for rpc messages in DefaultRpcPBMessageFactory");

void DefaultBaiduProxyPBMessages::Clear() {
    request.Clear();
    response.Clear();
}
...
RpcPBMessages* DefaultRpcPBMessageFactory::Get(
    ...
    if (FLAGS_enable_protobuf_arena_for_rpc) {
        messages->request = service.GetRequestPrototype(&method).New(&_arena);
        messages->response = service.GetResponsePrototype(&method).New(&_arena);
    } else {
...
RpcPBMessageFactory* GetDefaultRpcPBMessageFactory() {
    return butil::get_leaky_singleton<DefaultRpcPBMessageFactory>();
}
