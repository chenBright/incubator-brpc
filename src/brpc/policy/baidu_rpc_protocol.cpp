...
void SendRpcResponse(int64_t correlation_id,
    ...
    auto messages_guard = butil::MakeScopeGuard([server, messages] {
        ...
        if (NULL != server->options().baidu_master_service) {
            auto baidu_proxy_messages = static_cast<DefaultBaiduProxyPBMessages*>(messages);
            baidu_proxy_messages->Clear();
            butil::return_object(baidu_proxy_messages);
        } else {
...
void ProcessRpcRequest(InputMessageBase* msg_base) {
    ...
    do {
        ...
        if (NULL != server->options().baidu_master_service) {
            ...
            messages = butil::get_object<DefaultBaiduProxyPBMessages>();
            msg->payload.cutn(
